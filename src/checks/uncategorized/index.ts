...
export const checkUncategorizedMimeTypesInconsistent: Checker = {
  ...
  handler: async ({ log }) => {
    ...
    log.trace(
      BotRuntime.Uncategorized,
      'mime_types_inconsistent',
      `navigator.mimeTypes data type is inconsistent: ${isConsistent}`,
    )
...
export const checkUncategorizedEvalLengthInconsistent: Checker = {
  ...
  handler: async ({ log, browser, browserCore }) => {
    ...
    log.trace(
      BotRuntime.Uncategorized,
      'eval_length_inconsistent',
      `eval.toString().length is inconsistent: ${evalLength}`,
    )
