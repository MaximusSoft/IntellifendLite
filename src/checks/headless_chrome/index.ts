...
export const checkHeadlessAppVersion: Checker = {
    ...
    handler: async ({ log }) => {
        ...
        log.trace(BotRuntime.HeadlessChrome, 'app_version', `navigator.appVersion must not contains "headless": ${appVersion}`)
...
export const checkHeadlessLanguagesLengthInconsistent: Checker = {
    ...
    handler: async ({ log, browserCore }) => {
        ...
        log.trace(BotRuntime.HeadlessChrome, 'languages_length_inconsistent', `navigator.languages must contain an value: ${JSON.stringify(languages)}`)
...
export const checkNotificationPermissions: Checker = {
    ...
    handler: async ({ log, browser }) => {
        ...
        try {
            ...
            log.trace(BotRuntime.HeadlessChrome, 'notification_permissions', `permissionStatus must not be prompt: ${JSON.stringify(permissionStatus)}`)
            return window.Notification.permission === 'denied' && permissionStatus.state === 'prompt'
        } catch (e) {
            log.trace(BotRuntime.HeadlessChrome, 'notification_permissions', `notificationPermissions signal unexpected behaviour`)
            return false
        }
    }
}
...
export const checkHeadlessWindowProperties: Checker = {
    ...
    handler: async ({ log }) => {
        ...
        log.trace(BotRuntime.HeadlessChrome, 'headless_window_properties', `window properties must not contain "webdriver", "domAutomation", "domAutomationController": ${contains}`)
        return contains
    }
}
    ...
    handler: async ({ log, browserCore }) => {
        ...
        if (
          ...
          (browserCore === BrowserCore.Webkit && !isDesktopSafari())
            ...
            log.trace(BotRuntime.HeadlessChrome, 'plugins_inconsistent', `browserCore is Chromium in Android; or Webkit in iOS`)
            return false
        }
...
export const checkWeGL: Checker = {
    ...
    handler: async ({ log }) => {
        ...
        log.trace(BotRuntime.HeadlessChrome, 'webgl', `WebGLRenderingContext.Parameters{VENDOR,RENDERER} must not be {"Brian Paul","Mesa OffScreen"}: ${vendor}, ${renderer}`)
